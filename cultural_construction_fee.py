...
def _ccf_extract_vals(row, start_col, count):
    """从row提取count个有效数值，跳过栏次号"""
    vals = []
    for j in range(start_col, min(start_col + count + 5, len(row))):
        ...
        vals.append(_ccf_to_num(val))
        if len(vals) >= count: break
    while len(vals) < count: vals.append(0.0)
    return vals

def _parse_ccf_main_form(data_2d):
    """解析文化事业建设费申报表主表（19行）, 返回 {row1_taxable_income_current, row1_taxable_income_ytd, ...}"""
    ...
    for row in data_2d[:10]:
        ...
        for j, cell in enumerate(row):
            ...
            for k in range(j + 1, min(j + 4, len(row))):
                if row[k]: val = str(row[k]).strip(); break
    ...
    # 关键字 → (字段基础名)
    # 注意：长关键字必须排在短关键字前面，避免子串匹配错误
    # 例如 "应征收入减除额" 必须在 "应征收入" 前面
    mappings = [
        # 费额计算（减除相关长关键字优先）
        ("减除项目期初金额",     "row3_deduction_beginning"),
        ("减除项目本期发生额",   "row4_deduction_current_period"),
        ("减除项目期末余额",     "row7_deduction_ending_balance"),
        ("应征收入减除额",      "row5_taxable_income_deduction"),
        ("免征收入减除额",      "row6_tax_exempt_deduction"),
        # 费额缴纳（长关键字优先）
        ("期初未缴费额",        "row11_unpaid_beginning"),
        ("本期已缴费额",        "row12_paid_current_period"),
        ("本期预缴费额",        "row13_prepaid"),
        ("本期缴纳上期费额",     "row14_paid_last_period"),
        ("本期缴纳欠费额",      "row15_paid_arrears"),
        ("期末未缴费额",        "row16_unpaid_ending"),
        ("本期应补退费额",      "row18_fill_refund"),
        ("本期应补",            "row18_fill_refund"),
        ("本期检查已补缴费额",   "row19_inspected_supplement"),
        # 计费收入（短关键字放最后）
        ("应征收入",           "row1_taxable_income"),
        ("免征收入",           "row2_tax_exempt_income"),
        # 其他短关键字
        ("计费销售额",          "row8_taxable_sales"),
        ("应缴费额",            "row10_payable_fee"),
        ("欠缴费额",            "row17_arrears"),
        ("费率",               "row9_fee_rate"),
    ]
    ...
    for row in data_2d:
        if not row: continue
        # 预处理：如果row是单元素列表（来自pdfplumber文本行），拆分为多列
        if len(row) == 1 and isinstance(row[0], str) and row[0]:
            parts = row[0].split()
            if len(parts) >= 2:
                row = parts
        ...
        for col_idx, cell in enumerate(row):
            ...
            clean = _ccf_clean(str(cell))
            for kw, field_base in mappings:
                if kw in clean:
                    vals = _ccf_extract_vals(row, col_idx + 1, 2)
                    result[f"{field_base}_current"] = vals[0]
                    result[f"{field_base}_ytd"] = vals[1]
                    break
    
    # 费率默认0.03（如果没识别到）
    if "row9_fee_rate_current" not in result:
        result["row9_fee_rate_current"] = 0.03
        result["row9_fee_rate_ytd"] = 0.03
    result["fee_reduction_rate"] = 0.5
...
async def import_ccf_declaration(
    ...
    try:
                ...
                for row in data[:5]:
                    for cell in (row or []):
                        if cell and isinstance(cell, str) and len(cell.strip()) > 3:
                            stripped = cell.strip()
                            if any(kw in stripped for kw in ['文化', '建设费', '申报表', '扣除', '清单', '广告']):
            ...
            for row in data[:10]:
                ...
                for cell in row:
                    if cell and isinstance(cell, str):
                        for kw in keywords:
                            if kw in cell: return True
            return False
        
        for sh in sheets:
            sn = _sheet_title(sh)
            data = sh["data"]
            if _sheet_contains(sn, data, '扣除', 'deduction', '清单'):
                form_deduction = data
            elif _sheet_contains(sn, data, '主表', '申报表', '文化', '建设费'):
                form_main = data
        
        # 兜底：如果没匹配到主表，取最大表格当主表
        if not form_main and sheets:
            candidates = [(len(sh["data"]), sh["data"]) for sh in sheets if len(sh["data"]) > 5]
            if candidates:
        ...
        parsed_main = {}
        if form_main:
            parsed_main = _parse_ccf_main_form(form_main)
            parsed_main["_raw"] = form_main
        ...
        # 统计解析字段数
        parsed_fields = len([k for k in parsed_main if not k.startswith("_")])
