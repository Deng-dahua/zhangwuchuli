...
async def import_file_with_mapping(  # v2026-06-04-simplify: 进项发票改为单步导入
    ...
    try:
        ...
        # ── 导入完成，自动智能处理 ──
        # ① 确定供应商档案（双源）→ ② 取得发票序时账 → ③ 银行流水序时账 → ④ 社保缴纳匹配
        # 处理过程中自动创建缺失的会计科目（_ensure_account）
        try:
            supp_result = _do_auto_create_suppliers(db, company_id)
            db.commit()
            if supp_result["created"] > 0:
                infos.append(f"自动建档 {supp_result['created']} 个供应商")
        except Exception as e:
            infos.append(f"供应商建档失败: {str(e)}")
        try:
            pi_count = auto_generate_purchase_journal(db, company_id)
            db.commit()
            if pi_count > 0:
                infos.append(f"自动生成 {pi_count} 号取得发票凭证")
        except Exception as e:
            db.rollback()
            infos.append(f"取得发票凭证生成失败: {str(e)}")
        try:
            bank_result = _generate_bank_journals(db, company_id, None)
            db.commit()
            if bank_result["generated"] > 0:
                infos.append(f"自动生成 {bank_result['generated']} 号银行流水凭证")
        except Exception as e:
            db.rollback()
            infos.append(f"银行流水凭证生成失败: {str(e)}")
        try:
            _match_ss_payment_journals(db, company_id)
            db.commit()
        except Exception:
            pass
