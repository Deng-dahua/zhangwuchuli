...
async function renderSalesInvoices(container) {
  ...
  try {
    ...
    html += '<button onclick="document.getElementById(\'siKeyword\').value=\'\';siFilter.keyword=\'\';renderSalesInvoices()" style="padding:6px 10px;border:1px solid #d1d5db;border-radius:6px;background:#fff;cursor:pointer;color:#6b7280" title="清空搜索内容">✕</button>';
    html += '<span style="display:inline-flex;align-items:center;gap:2px">';
    html += '<input id="siDateFrom" type="text" placeholder="yyyy/mm/dd" value="' + fmtDate(siFilter.dateFrom) + '" onchange="siFilter.dateFrom=this.value.replace(/\\//g,\'-\');renderSalesInvoices()" style="padding:6px 10px;border:1px solid #d1d5db;border-radius:6px 0 0 6px;width:95px" title="起始日期">';
    ...
    html += '<span style="display:inline-flex;align-items:center;gap:2px">';
    html += '<input id="siDateTo" type="text" placeholder="yyyy/mm/dd" value="' + fmtDate(siFilter.dateTo) + '" onchange="siFilter.dateTo=this.value.replace(/\\//g,\'-\');renderSalesInvoices()" style="padding:6px 10px;border:1px solid #d1d5db;border-radius:6px 0 0 6px;width:95px" title="截止日期">';
