...
async function renderSalesInvoices(container) {
  ...
  try {
    ...
    html += '<input type="text" placeholder="起始日期" value="' + (siFilter.dateFrom ? siFilter.dateFrom.replace(/(\d{4})-(\d{2})-(\d{2})/,'$1年$2月$3日') : '') + '" onclick="this.type=\'date\';this.value=\''+(siFilter.dateFrom||'')+'\';this.showPicker()" onchange="siFilter.dateFrom=this.value;this.type=\'text\';this.value=this.value.replace(/(\d{4})-(\d{2})-(\d{2})/,\'$1年$2月$3日\');renderSalesInvoices()" onblur="if(this.type==\'date\'){this.type=\'text\';this.value=\'' + (siFilter.dateFrom ? siFilter.dateFrom.replace(/(\d{4})-(\d{2})-(\d{2})/,'$1年$2月$3日') : '') + '\'}" style="padding:6px 10px;border:1px solid #d1d5db;border-radius:6px;width:130px" title="起始日期">';
    html += '<span style="color:#9ca3af">至</span>';
    html += '<input type="text" placeholder="截止日期" value="' + (siFilter.dateTo ? siFilter.dateTo.replace(/(\d{4})-(\d{2})-(\d{2})/,'$1年$2月$3日') : '') + '" onclick="this.type=\'date\';this.value=\''+(siFilter.dateTo||'')+'\';this.showPicker()" onchange="siFilter.dateTo=this.value;this.type=\'text\';this.value=this.value.replace(/(\d{4})-(\d{2})-(\d{2})/,\'$1年$2月$3日\');renderSalesInvoices()" onblur="if(this.type==\'date\'){this.type=\'text\';this.value=\'' + (siFilter.dateTo ? siFilter.dateTo.replace(/(\d{4})-(\d{2})-(\d{2})/,'$1年$2月$3日') : '') + '\'}" style="padding:6px 10px;border:1px solid #d1d5db;border-radius:6px;width:130px" title="截止日期">';
