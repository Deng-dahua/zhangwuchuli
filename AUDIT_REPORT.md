...
| *(本轮)* | M5-VAT科目常量, M19-nullable约束, M22-backref→back_populates, M23-免税零金额分录, M25-幂等检查死代码, M26-bank journal ref_id |

### 待修复（剩31项）
**HIGH（2项）**：H3-H6 裸dict→Pydantic模型（需重构）、H9 Python层全量聚合（架构级）
**MEDIUM（7项）**：M7公积金比例透明化、M8期初余额硬编码、M9-M11 LLM代码嵌入、M12-M14 迁移逻辑死代码、M18重复函数、M24进项税category（非bug）、M27并发凭证号、M28-M31细节
**LOW（22项）**：死代码、注释、代码风格
